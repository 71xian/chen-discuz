 ...
 */
package com.aoexe.discuz.system.core.cache.base;
...
import com.aoexe.discuz.core.constant.Symbol;
...
public abstract class AbstractRedisCacheOperator<T> implements CacheOperator<T> {
	...
	public Collection<String> getAllKeys() {
		Set<String> keys = redisTemplate.keys(getCommonKeyPrefix() + Symbol.ASTERISK);
		if (keys != null) {
			// 去掉缓存key的common prefix前缀
			return keys.stream().map(key -> key.substring(getCommonKeyPrefix().length()))
